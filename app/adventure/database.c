...
void gettrav( int loc )
{
  ...
  if ( loc <= 0 || loc >= (int) cavesz )
    bug( 42 );
  if ( blimp_strlen( cave[loc - 1] ) > sizeof( atrav ) )
    bug( 43 );

  blimp_strcpy( atrav, cave[loc - 1] );
  while ( ( aptr = blimp_strrchr( atrav, ',' ) ) )
    *aptr = '\0'; /* terminate substring	*/
  aptr = &atrav[0];
  for ( i = 0; i < MAXTRAV - 1; ++i ) {
    t               = blimp_atol( aptr ); /* convert to long int	*/
...
int rdupto( FILE *fdi, char uptoc, char print, char *string )
{
  ...
  while ( ( c = blimp_getchar( fdi ) ) != uptoc ) {
    if ( c == EOF )
      return 0;
    if ( c == '\r' )
      continue;
...
void rdskip( FILE *fdi, char skipc, int n, char rewind )
{
  ...
  if ( rewind )
    if ( fseek( fdi, 0, SEEK_SET ) == -1 )
      bug( 31 );
  while ( n-- ) {
    while ( ( c = blimp_getchar( fdi ) ) != skipc ) {
      if ( c == EOF )
        bug( 32 );
    }
  }
...
int yes( int msg1, int msg2, int msg3 )
{
  ...
  blimp_fputs( "> " );
  blimp_fflush();
  if ( NULL == blimp_fgets( answer, 80 ) )
    blimp_exit( 0 );
  if ( blimp_tolower( answer[0] ) == 'n' ) {
    if ( msg3 )
      rspeak( msg3 );
    return 0;
  }
...
{
#ifdef BUILTIN
  blimp_fputs( adventtxt4[msg - 1] );
#else
  if ( msg == 54 )
    blimp_printf( "ok.\n" );
  else {
    if ( dbugflg )
      blimp_printf( "Seek loc msg #%d @ %d\n", msg, idx4[msg] );
...
int binary( char *w, struct wac wctable[], int maxwc )
{
  ...
  while ( lo <= hi ) {
    mid = ( lo + hi ) / 2;
    if ( ( check = blimp_strcmp( w, wctable[mid].aword ) ) < 0 )
      hi = mid - 1;
    else if ( check > 0 )
      lo = mid + 1;
    else
...
void bug( int n )
{
  blimp_printf( "Fatal error number %d\n", n );
  blimp_exit( -1 );
}
