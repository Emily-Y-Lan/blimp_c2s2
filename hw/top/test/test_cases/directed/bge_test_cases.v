...
task test_case_directed_bge_1_basic();
  t.test_case_begin( "test_case_directed_bge_1_basic" );
  ...
  check_trace( 'h204, 'x, 'x,          0 ); // bge  x1, x0, 0x20c
...
task test_case_directed_bge_2_taken();
  t.test_case_begin( "test_case_directed_bge_2_taken" );
  if( !t.run_test ) return;
  fl_reset();
...
task test_case_directed_bge_3_not_taken();
  t.test_case_begin( "test_case_directed_bge_3_not_taken" );
  if( !t.run_test ) return;
  fl_reset();
...
task test_case_directed_bge_4_chain();
  t.test_case_begin( "test_case_directed_bge_4_chain" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h218, "bge  x2,  x1, 0x220" );
  asm( 'h21c, "addi x7,  x0, 2"     );
  asm( 'h220, "bge  x4,  x3, 0x228" );
  asm( 'h224, "addi x8,  x0, 3"     );
  asm( 'h228, "bge  x6,  x5, 0x230" );
  ...
  check_trace( 'h218, 'x, 'x, 0 ); // bge  x2,  x1, 0x220
  check_trace( 'h220, 'x, 'x, 0 ); // bge  x4,  x3, 0x228
  check_trace( 'h228, 'x, 'x, 0 ); // bge  x6,  x5, 0x230
...
task test_case_directed_bge_5_backward();
  t.test_case_begin( "test_case_directed_bge_5_backward" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  check_trace( 'h210, 'x, 'x, 0 ); // bge   x1,  x0, 0x244
  check_trace( 'h244, 14, 11, 1 ); // addi x14,  x0, 11
  check_trace( 'h248, 'x, 'x, 0 ); // bge   x0, x15, 0x238
  check_trace( 'h238, 11,  9, 1 ); // addi x11,  x0, 9
  check_trace( 'h23c, 'x, 'x, 0 ); // bge   x0, x12, 0x228
  check_trace( 'h228,  7,  6, 1 ); // addi  x7,  x0, 6
  check_trace( 'h22c, 'x, 'x, 0 ); // bge   x8,  x0, 0x214
...
task test_case_directed_bge_6_loop();
  t.test_case_begin( "test_case_directed_bge_6_loop" );
  if( !t.run_test ) return;
  fl_reset();

  // Write assembly program into memory

  asm( 'h200, "addi x1, x0, 3"    ); //
  asm( 'h204, "addi x1, x1, -1"    ); // <-.
  asm( 'h208, "addi x1, x1, -1"    ); //   |
  asm( 'h20c, "bge  x1, x0, 0x204" ); // --'
  asm( 'h210, "addi x2, x0, 1"     ); //
  asm( 'h214, "addi x3, x0, 2"     ); //

  // Check each executed instruction

  check_trace( 'h200,  1,  3, 1 ); // addi x1, x0, -3
  check_trace( 'h204,  1,  2, 1 ); // addi x1, x1, 1
  check_trace( 'h208,  1,  1, 1 ); // addi x1, x1, 1
  check_trace( 'h20c, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  check_trace( 'h204,  1,  0, 1 ); // addi x1, x1, -1
  check_trace( 'h208,  1, -1, 1 ); // addi x1, x1, -1
  check_trace( 'h20c, 'x, 'x, 0 ); // bge  x1, x0, 0x204
...
task test_case_directed_bge_7_loop_self();
  t.test_case_begin( "test_case_directed_bge_7_loop_self" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h200, "addi x1, x0, 1"    );
  asm( 'h204, "bge  x1, x0, 0x204" );

  // Check each executed instruction

  check_trace( 'h200,  1,  1, 1 ); // addi x1, x0, 1
  check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bge  x1, x0, 0x204
