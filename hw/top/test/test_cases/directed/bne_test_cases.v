...
task test_case_directed_bne_1_basic();
  t.test_case_begin( "test_case_directed_bne_1_basic" );
  ...
  check_trace( 'h204, 'x, 'x,          0 ); // bne  x1, x0, 0x20c
...
task test_case_directed_bne_2_taken();
  t.test_case_begin( "test_case_directed_bne_2_taken" );
  if( !t.run_test ) return;
  fl_reset();
...
task test_case_directed_bne_3_not_taken();
  t.test_case_begin( "test_case_directed_bne_3_not_taken" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  check_trace( 'h214,  5, 100, 1 ); // addi x5,  x0, 100
  check_trace( 'h218,  6, 100, 1 ); // addi x6,  x0, 100
  check_trace( 'h21c, 'x,  'x, 0 ); // bne  x5,  x6, 0x224
...
task test_case_directed_bne_4_chain();
  t.test_case_begin( "test_case_directed_bne_4_chain" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h218, "bne  x1,  x2, 0x220" );
  asm( 'h21c, "addi x7,  x0, 2"     );
  asm( 'h220, "bne  x3,  x4, 0x228" );
  asm( 'h224, "addi x8,  x0, 3"     );
  asm( 'h228, "bne  x3,  x4, 0x230" );
  ...
  check_trace( 'h218, 'x, 'x, 0 ); // bne  x1,  x2, 0x220
  check_trace( 'h220, 'x, 'x, 0 ); // bne  x3,  x4, 0x228
  check_trace( 'h228, 'x, 'x, 0 ); // bne  x3,  x4, 0x230
...
task test_case_directed_bne_5_backward();
  t.test_case_begin( "test_case_directed_bne_5_backward" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h208, "addi x12, x0, 3"     );
  asm( 'h20c, "addi x15, x0, 4"     );

  asm( 'h210, "bne  x1,  x0, 0x244" ); // --------.
  ...
  asm( 'h22c, "bne  x8,  x0, 0x214" ); // --'  |  |
  asm( 'h230, "addi x9,  x0, 7"     ); //      |  |
  asm( 'h234, "addi x10, x0, 8"     ); //      |  |
  asm( 'h238, "addi x11, x0, 9"     ); // <-.  |  |
  asm( 'h23c, "bne  x12, x0, 0x228" ); // --+--'  |
  asm( 'h240, "addi x13, x0, 10"    ); //   |     |
  asm( 'h244, "addi x14, x0, 11"    ); // <-+-----'
  asm( 'h248, "bne  x15, x0, 0x238" ); // --'

  // Check each executed instruction

  check_trace( 'h200,  1, 1, 1 ); // addi x1,  x0, 1
  check_trace( 'h204,  8, 2, 1 ); // addi x8,  x0, 2
  check_trace( 'h208, 12, 3, 1 ); // addi x12, x0, 3
  check_trace( 'h20c, 15, 4, 1 ); // addi x15, x0, 4

  check_trace( 'h210, 'x, 'x, 0 ); // bne  x1,  x0, 0x244
  check_trace( 'h244, 14, 11, 1 ); // addi x14, x0, 11
  check_trace( 'h248, 'x, 'x, 0 ); // bne  x15, x0, 0x238
  check_trace( 'h238, 11,  9, 1 ); // addi x11, x0, 9
  check_trace( 'h23c, 'x, 'x, 0 ); // bne  x12, x0, 0x228
  check_trace( 'h228,  7,  6, 1 ); // addi x7,  x0, 6
  check_trace( 'h22c, 'x, 'x, 0 ); // bne  x8,  x0, 0x214
...
task test_case_directed_bne_6_loop();
  t.test_case_begin( "test_case_directed_bne_6_loop" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h204, "addi x1, x1, -1"    ); // <-.
  asm( 'h208, "addi x1, x1, -1"    ); //   |
  asm( 'h20c, "bne  x1, x0, 0x204" ); // --'
  asm( 'h210, "addi x2, x0, 1"     ); //
  asm( 'h214, "addi x3, x0, 2"     ); //

  // Check each executed instruction

  check_trace( 'h200,  1,  4, 1 ); // addi x1, x0, 4
  check_trace( 'h204,  1,  3, 1 ); // addi x1, x1, -1
  check_trace( 'h208,  1,  2, 1 ); // addi x1, x1, -1
  check_trace( 'h20c, 'x, 'x, 0 ); // bne  x1, x0, 0x204
  check_trace( 'h204,  1,  1, 1 ); // addi x1, x1, -1
  check_trace( 'h208,  1,  0, 1 ); // addi x1, x1, -1
  check_trace( 'h20c, 'x, 'x, 0 ); // bne  x1, x0, 0x204
  check_trace( 'h210,  2,  1, 1 ); // addi x2, x0, 1
  check_trace( 'h214,  3,  2, 1 ); // addi x3, x0, 2
...
task test_case_directed_bne_7_loop_self();
  t.test_case_begin( "test_case_directed_bne_7_loop_self" );
  if( !t.run_test ) return;
  fl_reset();
  ...
  asm( 'h200, "addi x1, x0, 1"     );
  asm( 'h204, "bne  x1, x0, 0x204" );

  // Check each executed instruction

  check_trace( 'h200,  1,  1, 1 ); // addi x1, x0, 1
  check_trace( 'h204, 'x, 'x, 0 ); // bne  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bne  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bne  x1, x0, 0x204
  check_trace( 'h204, 'x, 'x, 0 ); // bne  x1, x0, 0x204
